...
#### MetaCyc pathway ####
MetaCyc_abundance <- read_delim("filtered_picrust/path_abun_unstrat.tsv",delim = "\t", escape_double = FALSE, trim_ws = TRUE)  #fix this
...
# subset 30 because dataset too large
subset_MetaCyc <- head(signif_MetaCyc_daa_annotated_results_df, 30)

# error bar plot
pathway_errorbar(abundance = MetaCyc_abundance %>% column_to_rownames("pathway"), daa_results_df = subset_MetaCyc, Group = metadata$Horizon, ko_to_kegg = FALSE, p_values_threshold = 0.05, order = "group", select = NULL, p_value_bar = TRUE, colors = NULL, x_lab = "description")
